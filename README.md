...
# V12 mantém compatibilidade com o banco da V10
...
st.set_page_config(page_title="Manutenção V12 Industrial", page_icon="🏭", layout="wide")
...
def login_screen():
    header("🏭 Manutenção V12 Industrial", "Preventiva automática • Mantém base da V10 • Tesla MES")
...
header("⚙️ Centro de Manutenção Industrial", "Versão V12 pronta para fábrica")
if auto_msgs:
...
if page == "Dashboard":
    ...
    with c1:
        ...
        if not wo_df.empty and "maintenance_type" in wo_df.columns:
            tdf = wo_df["maintenance_type"].fillna("Corretiva").value_counts().reset_index()
            tdf.columns = ["Tipo", "Qtd"]
            fig = px.bar(tdf, x="Tipo", y="Qtd", text="Qtd")
    ...
    with c2:
        ...
        st.write(f"**Técnicos inativos:** {tech_inativos}")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if wo_df.empty:
        ...
        for _, row in panel_df.iterrows():
            st.markdown(f'<div class="panel" style="margin-bottom:14px;"><div style="display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap;"><div><div style="font-size:1.05rem;font-weight:800;">{row["os_number"]} • {row["machine_code"] or "-"} • {row.get("maintenance_type","Corretiva")}</div><div class="small-muted">{row["sector"]} • {row["criticality"]} • Solicitante: {row["requester"]}</div></div><div>{badge(row["status"])}</div></div></div>', unsafe_allow_html=True)
            ...
            with desc:
                ...
                if row["status"] in ["Em manutenção", "Pausada"]:
                    st.markdown('<div class="warn-box"><strong>Máquina em manutenção</strong></div>', unsafe_allow_html=True)
                elif row["status"] == "Preventiva":
                    st.markdown('<div class="ok-box"><strong>OS preventiva aguardando início</strong></div>', unsafe_allow_html=True)
            ...
            with act:
                a1, a2 = st.columns(2)
                with a1:
                    if row["status"] in ["Aberta","Máquina Parada","Preventiva"] and st.button(f"Iniciar {row['id']}", use_container_width=True):
                        start = now_str()
    ...
    require_gestor()
    st.markdown('<div class="panel">Módulo de cadastros mantido com a mesma lógica segura da V10.</div>', unsafe_allow_html=True)
    st.info("Use a V10/V11 como base visual de cadastros; esta V12 prioriza o fluxo industrial e a preservação do banco da V10.")
...
""")
    st.write("Esta V12 usa o banco da V10 por padrão para manter máquinas e técnicos já cadastrados.")
    st.write("Para preservar os cadastros, mantenha o arquivo `manutencao_v10_fabrica.db` no mesmo ambiente.")
    st.markdown("</div>", unsafe_allow_html=True)
