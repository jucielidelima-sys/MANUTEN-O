...
def build_kpis(wo_df, prev_df, parts_df, tech_df, machines_full_df):
    ...
    return total_os, abertas, manut, paradas, mttr, mtbf, custo_total, prev_venc, low_stock, tech_inativos, maq_inativas
...
header("⚙️ Centro de Manutenção Industrial", "Versão V10 pronta para fábrica")
if auto_msgs:
...
if page == "Dashboard":
    total_os, abertas, manut, paradas, mttr, mtbf, custo_total, prev_venc, low_stock, tech_inativos, maq_inativas = build_kpis(wo_df, prev_df, parts_df, tech_df, machines_full_df)
    cols = st.columns(6)
    data = [
        ...
        ("Téc. inativos", tech_inativos, "Equipe"),
    ...
    with c3:
        ...
        st.write(f"**MTTR:** {mttr} min")
        st.write(f"**MTBF:** {mtbf} h")
