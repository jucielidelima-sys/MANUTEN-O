...
.stApp{
  background:
    radial-gradient(circle at 15% 20%, rgba(0,255,214,0.08), transparent 18%),
    radial-gradient(circle at 85% 15%, rgba(92,200,255,0.10), transparent 20%),
    linear-gradient(135deg, rgba(6,9,14,1), rgba(2,5,8,1)),
    repeating-linear-gradient(45deg, rgba(255,255,255,0.015) 0px, rgba(255,255,255,0.015) 6px, rgba(0,0,0,0) 6px, rgba(0,0,0,0) 12px);
  color:#edf4fb;
}
.block-container{max-width:1540px;padding-top:1rem;padding-bottom:1.2rem;}
.header-box,.panel,.kpi{
  background:linear-gradient(180deg, rgba(16,24,34,0.96), rgba(8,13,20,0.99));
  border:1px solid rgba(92,200,255,0.20);
  border-radius:24px;
  box-shadow:0 16px 36px rgba(0,0,0,0.45),0 0 30px rgba(92,200,255,0.08), inset 0 0 0 1px rgba(255,255,255,0.03);
}
...
.big-tile{
  background:
    linear-gradient(180deg, rgba(20,29,41,0.98), rgba(10,16,23,1));
  ...
  box-shadow:
    ...
    0 0 28px rgba(92,200,255,0.10),
    inset 0 0 18px rgba(92,200,255,0.04);
}
...
.stButton>button,.stDownloadButton>button{
  border-radius:14px !important;border:1px solid rgba(92,200,255,0.28) !important;
  background:linear-gradient(180deg, rgba(23,34,47,1), rgba(10,17,24,1)) !important;
  color:white !important;font-weight:700 !important;
  box-shadow:0 0 16px rgba(92,200,255,0.10) !important;
}
</style>
...
def make_progress_bar(value, max_value, title, suffix="", color="#5cc8ff"):
    ...
    fig.update_layout(
        ...
        xaxis=dict(range=[0, max_value], showgrid=False, zeroline=False, visible=False),
        yaxis=dict(showgrid=False, visible=False)
    )
    return fig

def make_vertical_thermo(value, max_value, title):
    value = float(value or 0)
    max_value = max(float(max_value or 1), 1.0)
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "borderwidth":1,
            "bordercolor":"rgba(92,200,255,0.14)",
            "steps":[
                {"range":[0, max_value*0.35], "color":"rgba(118,255,209,0.20)"},
                {"range":[max_value*0.35, max_value*0.7], "color":"rgba(255,184,77,0.20)"},
                {"range":[max_value*0.7, max_value], "color":"rgba(255,107,107,0.24)"}
            ]
        }
    ))
    fig.update_layout(
...
def make_ring_kpi(value, max_value, title, suffix="", color="#76ffd1"):
    value = float(value or 0)
    max_value = max(float(max_value or 1), 1.0)
    pct = max(0.0, min(value / max_value, 1.0))
    ...
    fig.update_layout(
        ...
        margin=dict(l=10, r=10, t=40, b=10),
        annotations=[
            dict(text=title, x=0.5, y=0.62, showarrow=False, font=dict(size=16, color="#9fb4c8")),
            dict(text=f"{value:.0f}{suffix}", x=0.5, y=0.44, showarrow=False, font=dict(size=28, color="#edf4fb"))
        ]
    )
    return fig
...
def init_db():
    ...
    if user_count == 0:
        ...
        # se o banco veio de versão antiga com senha sem hash, corrige
        ...
        # se algum usuário padrão estiver faltando, recria
        existing = {r[0] for r in cur.execute("SELECT username FROM users").fetchall()}
        defaults = [
            ("operador", hash_password("1234"), "Operador", "Operador Padrão"),
            ("manutencao", hash_password("1234"), "Manutenção", "Equipe de Manutenção"),
            ("gestor", hash_password("1234"), "Gestor", "Gestor da Manutenção"),
        ]
        for username, password, profile, full_name in defaults:
            if username not in existing:
                cur.execute(
                    "INSERT INTO users (username,password,profile,full_name) VALUES (?,?,?,?)",
                    (username, password, profile, full_name)
                )
    ...
    with b:
        ...
        if st.button("Acessar", use_container_width=True):
            ...
            if user:
                ...
                st.error("Usuário ou senha inválidos. Se esta for a primeira execução sobre base antiga, reinicie o app uma vez.")
...
header("⚙️ Centro de Manutenção Industrial", "Versão V12.3 SCADA / MES Premium")
if auto_msgs:
...
if page == "Dashboard":
    ...
    with g1:
        st.markdown('<div class="panel">', unsafe_allow_html=True)
        st.plotly_chart(make_progress_bar(mttr, max(mttr*1.5 if mttr else 120, 120), "MTTR", " min", "#5cc8ff"), use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
    with g2:
        st.markdown('<div class="panel">', unsafe_allow_html=True)
        st.plotly_chart(make_ring_kpi(mtbf, max(mtbf*1.4 if mtbf else 300, 300), "MTBF", " h", "#76ffd1"), use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
    with g3:
        st.markdown('<div class="panel">', unsafe_allow_html=True)
        st.plotly_chart(make_vertical_thermo(paradas, max(paradas+3, 5), "Máquinas paradas"), use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
