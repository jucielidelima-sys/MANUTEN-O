...
def send_whatsapp(msg, targets):
    instance_id = sec("ULTRAMSG_INSTANCE_ID")
    token = sec("ULTRAMSG_TOKEN")
    base_url = sec("ULTRAMSG_BASE_URL", "https://api.ultramsg.com").rstrip("/")
    if not instance_id or not token or not targets:
        return False, "Credenciais ou destinos não configurados."
    try:
        sent = 0
        errors = []
        for target in targets:
            number = str(target).replace("whatsapp:", "").replace(" ", "").strip()
            if not number:
                continue
            if not number.startswith("+"):
                number = f"+{number}"
            resp = requests.post(
                f"{base_url}/{instance_id}/messages/chat",
                data={
                    "token": token,
                    "to": number,
                    "body": msg,
        ...
        if sent > 0:
            detail = f"WhatsApp enviado para {sent} destino(s)."
            if errors:
                detail += " | Algumas falhas: " + " | ".join(errors[:2])
            return True, detail
        return False, " | ".join(errors) if errors else "Falha ao enviar WhatsApp."
    except Exception as e:
...
ULTRAMSG_INSTANCE_ID = "sua_instance_id"
ULTRAMSG_TOKEN = "seu_token"
ULTRAMSG_BASE_URL = "https://api.ultramsg.com"

WHATSAPP_MANUTENCAO = "554691144902"
WHATSAPP_GESTAO = "554691144902"
